...
import { renderProductIcon } from '../shared/product-icons.mjs';
...
function renderComponent(c) {
  ...
  const hasIcon = c.icon != null && c.icon !== '';
  const labelY = hasIcon
    ? (hasSub ? c.y + 52 : c.y + 56)
    : (hasSub ? c.y + c.height / 2 - 2 : c.y + c.height / 2 + 4);
  const subY = hasIcon ? c.y + 68 : c.y + c.height / 2 + 14;
  const sub = hasSub
  ...
  const icon = hasIcon ? renderProductIcon(c.icon, { x: cx - 13, y: c.y + 8, size: 26, tone: c.type }) : '';
  ...
  return `        <g ${focusNodeAttrs(c.id, c.label, passport, arch.meta.locale)}>
          ...
          ${hasIcon ? '' : renderSemanticSigil(c.type, { x: c.x + 6, y: c.y + 6 })}${icon ? `\n          ${icon}` : ''}${brand ? `\n          ${brand}` : ''}
