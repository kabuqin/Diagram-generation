...
- **Product icons on every node** — optional per-component "icon" field renders an authored line-art logo (12 built-in: user, cdn, ddos, waf, firewall, bastion, slb, waf-cloud, ecs, vpc-firewall, rds, oss) centered above the node label, tinted by the node's semantic type
